...
class VendorAdminApi extends WooCommerce {
  ...
  Future<List<Order>> getVendorAdminOrders(
    ...
    try {
      ...
      var endpoint =
          '${serverConfig['url']}/wp-json/vendor-admin/vendor-orders?page=$page&per_page=$perPage&token=$base64Str';
      if (status != null) {
  ...
  Future<List<Category>> getVendorAdminCategoriesByPage(
    ...
    try {
      ...
      var response = await http.get(url.toUri());
      var result = convert.jsonDecode(response.body);
      if (result is Map &&
          result['message'] != null &&
  ...
  Future<void> removeProduct({cookie, String productId}) async {
    try {
      var base64Str = Utils.encodeCookie(cookie);
      var endpoint =
          "${serverConfig['url']}/wp-json/vendor-admin/products?token=$base64Str&id=$productId";
      await http.delete(endpoint.toUri());
    } catch (e) {
      printLog('vendor_admin.dart removeProduct: $e');
    }
  }

  Future<List<ProductAttribute>> getDefaultProductAttributes({cookie}) async {
  ...
  Future updateOrder(orderId, {status, customerNote, token}) async {
    try {
      ...
      await http.put(endpoint.toUri(), body: {
