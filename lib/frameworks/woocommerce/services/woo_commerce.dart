...
class WooCommerce extends BaseServices {
  ...
  Future<User> loginFacebook({String token}) async {
    ...
    try {
      ...
      var response = await http.get(endPoint.toUri());
  ...
  Future<User> loginApple({String token}) async {
    try {
      ...
      var response = await http.post(endPoint.toUri(),
...
//          }
  ...
  Future<String> getCheckoutUrl(
    ...
    try {
      ...
      final response =
          await http.post('$url/wp-json/api/flutter_user/checkout'.toUri(),
              body: convert.jsonEncode({
                'order': base64Str,
              }),
              headers: {'Content-Type': 'application/json'});
  ...
  Future<Map<String, dynamic>> getCurrencyRate() async {
    try {
      final response = await http
          .get('$url/wp-json/api/flutter_user/get_currency_rates'.toUri());
      var body = convert.jsonDecode(response.body);
  ...
  Future getCountries() async {
    try {
      final response =
          await http.get('$url/wp-json/api/flutter_user/get_countries'.toUri());
      var body = convert.jsonDecode(response.body);
      return body;
    } catch (err) {
      rethrow;
  ...
  Future getCartInfo(String token) async {
    try {
      ...
      final response = await http
          .get('$url/wp-json/api/flutter_woo/cart?token=$base64Str'.toUri());
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
        return body;
      } else if (body['message'] != null) {
  ...
  Future<Point> getMyPoint(String token) async {
    try {
      var base64Str = Utils.encodeCookie(token);
      final response = await http
          .get('$url/wp-json/api/flutter_woo/points?token=$base64Str'.toUri());
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
  ...
  Future<List<Product>> getProductNearest(location) async {
    try {
      ...
      final response = await http.get(urlReq.toUri());
      if (response.statusCode == 200) {
        for (var item in convert.jsonDecode(response.body)) {
          ...
          for (var item in product.images) {
            if (!item.contains('http')) {
              var res =
                  await http.get('$url/wp-json/wp/v2/media/$item'.toUri());
  ...
  Future<List<StaffBookingModel>> getListStaff(String idProduct) async {
    ...
    final response = await http.get(urlAPI.toUri());
