...
class Ads with ChangeNotifier {
  ...
  static void googleAdInit() {
    if (!kIsWeb) {
      FirebaseServices().adMob?.initialize(
            appId:
                isAndroid ? kAdConfig['androidAppId'] : kAdConfig['iosAppId'],
          );
  ...
  static void showRewardedVideoAd() {
    if (!kIsWeb) {
      FirebaseServices().rewardedVideoAd?.load(
          adUnitId: isAndroid
      ...
      Future.delayed(Duration(seconds: kAdConfig['waitingTimeToDisplayReward']),
          () async {
        await FirebaseServices().rewardedVideoAd?.show();
      });
    }
  }
