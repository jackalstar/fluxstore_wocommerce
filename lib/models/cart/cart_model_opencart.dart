...
class CartModelOpencart
  ...
  String addProductToCart({
    ...
    if (message.isEmpty) {
      ...
      productVariationInCart[key] = ProductVariation.fromJson({'id': key, 'price': product.price, 'in_stock': true, 'image':{}});
      ...
      if (isSaveLocal) {
        saveCartToLocal(
            product: product,
            quantity: quantity,
            variation: ProductVariation.fromJson({'id': key, 'price': product.price, 'in_stock': true, 'image':{}}),
            options: {...options});
      }
    }
