...
import '../../../models/index.dart' show AppModel, Order, OrderNote, UserModel;
...
class OrderHistoryDetailScreen extends StatefulWidget {
  ...
  OrderHistoryDetailScreen({@required this.order});
...
class _OrderHistoryDetailScreenState extends State<OrderHistoryDetailScreen> {
  ...
  void cancelOrder() {
    Services().widget.cancelOrder(context, order).then((onValue) {
      Provider.of<ListOrderHistoryModel>(context, listen: false).refresh();
      setState(() {
        order = onValue;
      });
    });
          ...
          children: <Widget>[
            ...
            if (kPaymentConfig['ShowOrderNotes'] ?? true)
              Padding(
                ...
                child: FutureBuilder<List<OrderNote>>(
                  future: services.api
  ...
  Future<void> refundOrder() async {
    ...
    try {
      ...
      _hideLoading();
      await Provider.of<ListOrderHistoryModel>(context, listen: false)
          .refresh();
      Navigator.of(context).pop();
    } catch (err) {
      _hideLoading();
