...
class CategoryIcons extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var item in config['items']) {
      ...
      var name = item['title'] ?? '';
      if (categoryList[id] != null && !(item['keepDefaultTitle'] ?? false)) {
        name = categoryList[id].name;
      }
      if (config['hideTitle'] ?? false) {
        name = '';
      }
    ...
    if (config['wrap'] == false && items.isNotEmpty) {
      return SingleChildScrollView(
        ...
        padding: EdgeInsets.only(
          left: Tools.formatDouble(config['marginLeft'] ?? _paddingList),
          right: Tools.formatDouble(config['marginRight'] ?? _paddingList),
          top: Tools.formatDouble(config['marginTop'] ?? 12.0),
          bottom: Tools.formatDouble(config['marginBottom'] ?? 6.0),
        ),
        child: Row(
    ...
    return Container(
      ...
      child: Container(
        margin: EdgeInsets.only(
          ...
          bottom: Tools.formatDouble(config['marginBottom'] ?? 25.0),
        ),
